...
def execute_fold_parallel(participants_fold: pd.Series, fold: int, cuda_device: str,
                          hyper_parameters_tune_mode: bool=False, model_nums_list: list=None, reversed_order: bool=False):
    """
    ...
    for feature_num, bert_feature in bert_models:
        for model_num in all_model_nums:  # compare all versions of each model type
            ...
            for index, row in model_type_versions.iterrows():  # iterate over all the models to compare
                ...
                # if bert_feature == 'usinge_extreme_positive_words_3':
                #     continue
                model_name = f'{model_name}_{bert_feature}'
                data_file_name = data_file_name.replace('bert_embedding', f'bert_embedding_for_feature_{bert_feature}')
                test_data_file_name =\
...
def parallel_main(model_nums_list: list=None, reversed_order: bool=False):
...
if __name__ == '__main__':
    ...
    if is_parallel:
        parallel_main(model_nums_list=outer_model_nums_list, reversed_order=outer_reversed_order)
    else:
